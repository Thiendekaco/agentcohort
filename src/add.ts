...
const ARCHETYPE_META: Record<AgentArchetype, ArchetypeMeta> = {
  ...
  gate: {
    ...
    body: [
      ...
      '# Method',
      '',
      '1. Print a compact approval summary that states: what the human is',
      '   approving, the current conclusion, the next steps if approved,',
      '   what is not done yet, and the exact decision needed.',
      '2. Present explicit options via `AskUserQuestion`. Never use text',
      '   menus like `[y/revise/abort]`.',
      '3. Wait for the answer; do not act on assumptions.',
      '',
      '# Boundaries',
      '',
