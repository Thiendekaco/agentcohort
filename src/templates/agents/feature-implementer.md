...
4. Memory layer (agentcohort v0.10+).
   ...
   - Architecture choice → `agentcohort memory write decisions --json-body='{...}' --source=feature-implementer ...`
   - Verified bug fix → `agentcohort memory write bugs --json-body='{...}' --source=feature-implementer ...`
   ...
   - `agentcohort gate record --run-id=<RUN_ID> --gate=<name> --outcome=<verb> \\\
     --proposed-content="<short summary>" --posing-agent=feature-implementer [--reason="<user text>"]`
   - REQUIRED on every gate fire — approve, reject, escalate, auto-skip.

   When you VERIFY (or REFUTE) an earlier memory entry:
   - `agentcohort memory write verifications --json-body='{...}' --source=feature-implementer ...`
   ...
   **NEVER store secrets** — API keys, tokens, .env content, private keys,
   ...
   At the VERY START of your work, BEFORE reading any memory:
     `agentcohort run start --stage=feature-implementer --run-id=<RUN_ID>`

   At the VERY END, AFTER your last memory write:
     `agentcohort run end --stage=feature-implementer --run-id=<RUN_ID> --outcome=<success|failed|aborted>`
