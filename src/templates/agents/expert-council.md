...
4. Memory layer (agentcohort v0.10+).
   ...
   - Reads: decisions, bugs, scratch
   - Writes: scratch, audit
   ...
   - Architecture choice → `agentcohort memory write decisions --json-body='{...}' --source=expert-council ...`
   - Verified bug fix → `agentcohort memory write bugs --json-body='{...}' --source=expert-council ...`
   ...
   - `agentcohort gate record --run-id=<RUN_ID> --gate=<name> --outcome=<verb> \\\
     --proposed-content="<short summary>" --posing-agent=expert-council [--reason="<user text>"]`
   - REQUIRED on every gate fire — approve, reject, escalate, auto-skip.

   When you VERIFY (or REFUTE) an earlier memory entry:
   - `agentcohort memory write verifications --json-body='{...}' --source=expert-council ...`
   ...
   **NEVER store secrets** — API keys, tokens, .env content, private keys,
   ...
   At the VERY START of your work, BEFORE reading any memory:
     `agentcohort run start --stage=expert-council --run-id=<RUN_ID>`

   At the VERY END, AFTER your last memory write:
     `agentcohort run end --stage=expert-council --run-id=<RUN_ID> --outcome=<success|failed|aborted>`
