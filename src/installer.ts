...
export async function runInit(options: InitOptions): Promise<InitResult> {
  ...
  if (manifest.length === 0) {
    throw new Error('agent-force: no templates found to install.');
  }
  ...
  const decide = async (
    ...
    if (!options.resolver) {
      throw new Error('agent-force: interactive mode requires a resolver.');
    }
  ...
  const record = (a: ActionRecord) => {
    ...
    switch (a.disposition) {
      ...
      case 'appended-section':
        log.success(`${tag}append  ${a.targetRelPath} (Agent Force Routing Rules)`);
        break;
      case 'replaced-section':
        log.success(`${tag}update  ${a.targetRelPath} (Agent Force Routing Rules)${bk}`);
        break;
      case 'unchanged':
